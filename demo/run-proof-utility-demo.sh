...
node dist/cli.js kind "npm run package:smoke" >"$tmp_dir/kind-smoke.txt"
node dist/cli.js hash "$(cat fixtures/evidence.txt)" >"$tmp_dir/evidence.sha256"

grep -q '^test$' "$tmp_dir/kind-test.txt"
grep -q '^smoke$' "$tmp_dir/kind-smoke.txt"
test "$(wc -c <"$tmp_dir/evidence.sha256" | tr -d ' ')" -eq 65

echo "Command kind outputs: $tmp_dir/kind-test.txt, $tmp_dir/kind-smoke.txt"
